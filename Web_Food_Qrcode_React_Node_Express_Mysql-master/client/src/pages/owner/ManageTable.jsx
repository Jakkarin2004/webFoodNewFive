...
const handleSubmit = async () => {
  ...
  try {
    if (editingTable) {
      ...
      await axios.put(`${API_URL_TEBLE}/${editingTable.table_id}`, updatedTable);
      // อัปเดต state หลังจาก update สำเร็จ
      setTables(tables.map(table =>
        table.table_id === editingTable.table_id ? { ...table, ...updatedTable } : table
      ));
    } else {
    ...
    resetForm();
    alert(editingTable ? "แก้ไขโต๊ะสำเร็จ" : "เพิ่มโต๊ะใหม่สำเร็จ");
  } catch (error) {
  ...
  if (!window.confirm("คุณต้องการลบโต๊ะนี้ใช่หรือไม่?")) return;

  try {
    await axios.delete(`${API_URL_TEBLE}/${id}`);
    setTables(prev => prev.filter(table => table.table_id !== id));
    alert("✅ ลบโต๊ะสำเร็จ");
  } catch (err) {
    ...
    const handleDownloadQR = (table) => {
        ...
        alert(`ดาวน์โหลด QR Code สำหรับ ${table.table_name || table.table_number}`);
    };
                ...
                <div className="bg-white rounded-2xl shadow-xl mb-6 p-6 border-l-4 border-orange-500">
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-gray-600">เพิ่ม แก้ไข และสร้าง QR Code สำหรับโต๊ะในร้าน</p>
                        </div>
                        <button
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="font-semibold text-orange-800">QR Code</span>
                                    </div>
                                    <p className="text-sm text-orange-700">
                                        QR Code จะถูกสร้างอัตโนมัติเมื่อบันทึกโต๊ะใหม่ ลูกค้าสามารถสแกนเพื่อสั่งอาหารได้
                                    </p>
                                </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
                            ...
                            <div className="p-6 text-center">
                                <div className="mb-4">
                                    ...
                                    <p className="text-sm text-gray-600">หมายเลข: {selectedTable.table_number}</p>
                                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                    <div className="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-orange-500">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-semibold text-gray-600">โต๊ะทั้งหมด</p>
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-green-500">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-semibold text-gray-600">QR Code ที่สร้างแล้ว</p>
                                <p className="text-3xl font-bold text-gray-800">{tables.length}</p>
                            </div>
